...
export class ApplicationController {
    ...
    async bulkCreate(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if(!Data.length) throw new AppError(RES_TYPES.SOMETHING_WRONG, ERRORTYPES.INVALID_REQUEST);
            return res.status(201).json(
                { success: true, StatusCode: 201, data: Data, message: RES_TYPES.CREATE }
            );
        }
    ...
    async update(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if(!data || data === 0) throw new AppError(RES_TYPES.ID_NOT_FOUND,ERRORTYPES.NOT_FOUND);
            return res.json(
    ...
    async delete(req: Request, res: Response, next: NextFunction) {
        ...
        try {
            ...
            if (deleted) {
                ...
                return next(new AppError(RES_TYPES.ID_NOT_FOUND, ERRORTYPES.NOT_FOUND));
            }
        } catch (err) {
            return next(err);
        }
