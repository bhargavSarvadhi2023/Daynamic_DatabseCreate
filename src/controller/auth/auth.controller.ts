...
class AuthController {
    ...
    async forgetPassword(req, res, next) {
        try {
            ...
            const checkotp = await db.otpModel.findOne({
                where: { userId: user.id },
            });
            if (checkotp) {
                saveOtp = await db.otpModel.update(
                    { otp, expiresIn },
                    { where: { userId: user.id } },
                );
            } else {
                saveOtp = await db.otpModel.create({
                    otp,
                    expiresIn,
                    userId: user.id,
                });
    ...
    async resetPassword(req, res, next) {
        try {
            ...
            const [updatePassword] = await db.userModel.update(
                { password },
            ...
            const deleteOtp = await db.otpModel.destroy({
